 ...
 * util.php.
 *
 * http://brandonwamboldt.github.io/utilphp/s
 *
 ...
 *
 * @see http://github.com/brandonwamboldt/utilphp/ Official Documentation
 */
class Util
{
    /**
    ...
    public static function slugify($string, $separator = '-', $css_mode = false)
    {
        ...
        if (true === $separator || false === $separator) {
            $css_mode = $separator;
            $separator = '-';
    ...
    {
        $fixdSerializedData = preg_replace_callback(
            '!s:(\d+):"(.*?)";!',
            function ($matches) {
                $snip = $matches[2];
    ...
    public static function is_https($trust_proxy_headers = false)
    {
        ...
        if (isset($_SERVER['HTTPS']) && !empty($_SERVER['HTTPS']) && 'off' != $_SERVER['HTTPS']) {
            return isset($_SERVER['HTTPS']) && !empty($_SERVER['HTTPS']) && 'off' != $_SERVER['HTTPS'];
        }

        // Check proxy headers if allowed
        return $trust_proxy_headers && isset($_SERVER['X-FORWARDED-PROTO']) && 'https' == $_SERVER['X-FORWARDED-PROTO'];
    ...
    public static function add_query_arg($newKey, $newValue = null, $uri = null)
    {
        ...
        } elseif (isset($puri['path']) && false !== strstr($puri['path'], '=')) {
    ...
    public static function http_build_url($url, $parts = array(), $flags = self::HTTP_URL_REPLACE, &$new_url = array())
    {
        ...
        foreach ($keys as $key) {
            ...
            if ($flags & constant('WarezAddict\\util::'.$strip)) {
                unset($url[$key]);
            }
        }
    ...
    {
        return 0 === strpos($string, $starts_with);
    }
        ...
        if ('0' == $number[0]) {
            ...
            for ($z = 0; $z < count($groups2); ++$z) {
                if ('' != $groups2[$z]) {
                    ...
                    $output .= ($z < $length && !array_search('', array_slice($groups2, $z + 1, -1)) && '' != $groups2[$length] && '0' == $groups[$length][0] ? ' and ' : ', ');
                }
            }
    ...
    public static function get_current_url()
    {
        ...
        if ($is_https && (443 != $port)) {
            $url .= ':'.$_SERVER['SERVER_PORT'];
        } elseif (!$is_https && (80 != $port)) {
            $url .= ':'.$_SERVER['SERVER_PORT'];
        }
    ...
    public static function safe_truncate($string, $length, $append = '...')
    {
        ...
        if (false !== $last_space && $string != $ret) {
     ...
     * @param bool  $appendIntegerKeys Whether to append elements of $src
